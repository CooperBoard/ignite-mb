...
/* ═══════════════════════════════════════════════════════════════════
   IGNITE MB — FAITHFUL SITE COPY (RESPONSIVE)
...
const NAV = [
  ...
  { label: "About", page: null, children: [
  ...
  { label: "Ministries", page: null, children: [
...
function useScreen() {
  ...
  return { mobile: w < 768, tablet: w >= 768 && w < 1024, compact: w < 1024 };
}
...
function ContactForm({ heading, sub, btn = "Send" }) {
  ...
  return (
    <div style={{ padding: mobile ? "40px 20px" : "60px 40px", background: C.bgAlt, textAlign: "center" }}>
      {heading && <h2 style={{ fontSize: mobile ? 22 : 28, fontWeight: 700, margin: "0 0 6px", color: C.white }}>{heading}</h2>}
...
function Hero({ title, sub }) {
  const { mobile } = useScreen();
  return (
    <div style={{ padding: mobile ? "50px 20px 40px" : "70px 40px 50px", textAlign: "center", background: C.bg }}>
      <h1 style={{ fontSize: mobile ? 28 : 40, fontWeight: 700, margin: 0, color: C.white }}>{title}</h1>
      {sub && <p style={{ color: C.offWhite, fontSize: mobile ? 15 : 17, marginTop: 14, maxWidth: 700, margin: "14px auto 0", lineHeight: 1.7 }}>{sub}</p>}
    </div>
  );
}
...
function TimesGrid() {
  const { mobile } = useScreen();
  return (
    <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr 1fr", gap: 20, maxWidth: 800, margin: "0 auto" }}>
      {SITE.serviceTimes.map((s, i) => (
        <div key={i} style={{ background: C.bgLight, borderRadius: 8, padding: "28px 20px", border: `1px solid ${C.border}`, textAlign: "center" }}>
...
function Navbar({ page, go }) {
  ...
  return (
    <nav style={{
      background: C.bgDark, padding: compact ? "10px 16px" : "10px 28px",
      display: "flex", alignItems: "center", justifyContent: "space-between",
      flexWrap: "wrap", borderBottom: `1px solid ${C.border}`,
    }}>
      <div onClick={() => nav("home")} style={{ cursor: "pointer" }}>
        <img src={SITE.logo} alt="Ignite MB" style={{ height: compact ? 36 : 44 }} />
      </div>
      {compact && (
      ...
      {(!compact || menuOpen) && (
        <div style={{
          ...
          width: compact ? "100%" : "auto", marginTop: compact ? 10 : 0,
          flexWrap: compact ? "nowrap" : "wrap",
          ...(compact && { borderTop: `1px solid ${C.border}`, paddingTop: 8 }),
        }}>
          {NAV.map(item => (
            <div key={item.label} style={{ position: "relative" }}>
              <a onClick={(e) => {
                ...
                cursor: "pointer", padding: compact ? "10px 12px" : "8px 10px",
              ...
              {item.children && drop === item.label && (
                <div style={{
                  position: compact ? "static" : "absolute", top: "100%", left: 0,
                  background: C.bgDark, borderRadius: 6, padding: "4px 0",
                  minWidth: 220, zIndex: 100,
                  boxShadow: compact ? "none" : "0 6px 24px rgba(0,0,0,.5)",
                  border: `1px solid ${C.border}`,
                  ...(compact && { marginLeft: 18, border: "none" }),
                }}>
...
function Footer() {
  ...
  return (
    <footer style={{ background: C.footerBg, padding: mobile ? "36px 20px 20px" : "48px 40px 24px", borderTop: `1px solid ${C.border}` }}>
      <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr 1fr", gap: mobile ? 28 : 40, maxWidth: 960, margin: "0 auto" }}>
        <div>
...
function HomePage({ go }) {
  ...
  return (
    <>
      <div style={{ padding: mobile ? "60px 20px" : "90px 40px", textAlign: "center", background: C.bg }}>
        <h1 style={{ fontSize: mobile ? 32 : 48, fontWeight: 700, margin: "0 0 20px", color: C.white }}>Welcome</h1>
        <p style={{ color: C.offWhite, fontSize: mobile ? 15 : 17, maxWidth: 700, margin: "0 auto 30px", lineHeight: 1.7, fontWeight: 500 }}>
      ...
      <div style={{ padding: mobile ? "40px 20px" : "60px 40px", background: C.bgAlt, textAlign: "center" }}>
        <h2 style={{ fontSize: mobile ? 22 : 30, fontWeight: 700, margin: "0 0 8px", color: C.white }}>Latest Sermon</h2>
      ...
      <div style={{ padding: mobile ? "40px 20px" : "60px 40px", background: C.bg, textAlign: "center" }}>
        <h2 style={{ fontSize: mobile ? 22 : 30, fontWeight: 700, margin: "0 0 30px", color: C.white }}>Service Times</h2>
        <TimesGrid />
      </div>

      <div style={{ padding: mobile ? "40px 20px" : "60px 40px", background: C.bgAlt, textAlign: "center" }}>
        <h2 style={{ fontSize: mobile ? 22 : 28, fontWeight: 700, margin: "0 0 8px", color: C.white }}>Prayer Request</h2>
...
function ImNewPage() {
  ...
  return (
    <>
      ...
      <div style={{ padding: mobile ? "40px 20px" : "60px 40px", background: C.bgAlt, textAlign: "center" }}>
        <h2 style={{ fontSize: mobile ? 22 : 28, fontWeight: 700, margin: "0 0 28px", color: C.white }}>When We Meet</h2>
        <TimesGrid />
      </div>
      <div style={{ padding: mobile ? "40px 20px" : "60px 40px", background: C.bg }}>
        <h2 style={{ fontSize: mobile ? 22 : 28, fontWeight: 700, margin: "0 0 20px", textAlign: "center", color: C.white }}>What To Expect</h2>
        <div style={{ maxWidth: 650, margin: "0 auto" }}>
          {["Sunday morning we have spirit led worship.","Casual dress and Contemporary music is led by our worship team.","There is childcare available and activities for all age groups.","Plan to hang around after the service for fellowship and meeting new people.","Wednesday evenings we have light food and fellowship followed by prayer and Bible Study."].map((tx, i) => <p key={i} style={{ color: C.offWhite, fontSize: 15, lineHeight: 1.7, margin: "0 0 10px" }}>{tx}</p>)}
        </div>
      </div>
...
function MissionPage() {
  ...
  return (
    <>
      ...
      <div style={{ padding: mobile ? "40px 20px" : "60px 40px", background: C.bgAlt }}>
        <div style={{ maxWidth: 740, margin: "0 auto" }}>
          <h2 style={{ fontSize: mobile ? 20 : 26, fontWeight: 700, marginBottom: 16, color: C.white }}>The Mission & Process</h2>
      ...
      <div style={{ padding: mobile ? "40px 20px" : "60px 40px", background: C.bg }}>
        <div style={{ maxWidth: 740, margin: "0 auto" }}>
          <h2 style={{ fontSize: mobile ? 20 : 26, fontWeight: 700, marginBottom: 16, color: C.white }}>The Vision</h2>
          <p style={{ color: C.offWhite, fontSize: 15, lineHeight: 1.7 }}><strong>Planting Churches</strong> - A call to plant churches along the Grand Strand and the 10/40 window where most unreached people groups live. This will be done through making and multiplying disciples and starting new churches out of necessity.</p>
          <p style={{ color: C.offWhite, fontSize: 15, lineHeight: 1.7, marginTop: 14 }}><strong>Touching Our City</strong> - There is ONE church with many different names and locations. We are striving for unity in the body of Christ. We work with other churches and organizations.</p>
...
function BelievePage() {
  ...
  return (
    <>
      ...
      <div style={{ padding: mobile ? "40px 20px" : "60px 40px", background: C.bgAlt }}>
        <div style={{ maxWidth: 740, margin: "0 auto" }}>
          <h2 style={{ fontSize: mobile ? 22 : 28, fontWeight: 700, marginBottom: 8, textAlign: "center", color: C.white }}>Core Beliefs</h2>
          <p style={{ color: C.muted, fontSize: 14, textAlign: "center", marginBottom: 32 }}>The following are the core beliefs of Ignite MB based on the foundational truths taught in the Bible. All of our teaching and ministry is rooted in and flows out of these biblical doctrines.</p>
          {beliefs.map((b, i) => (
...
function StoryPage() {
  return (
    <>
      ...
      <div style={{ padding: "40px 20px", background: C.bgAlt }}>
        <div style={{ maxWidth: 740, margin: "0 auto" }}>
          {[
...
function ContactPage() {
  ...
  return (
    <>
      ...
      <div style={{ padding: mobile ? "40px 20px" : "60px 40px", background: C.bg, textAlign: "center" }}>
        <h2 style={{ fontSize: mobile ? 24 : 32, fontWeight: 700, margin: "0 0 20px", color: C.white }}>Contact Us</h2>
        <p style={{ color: C.offWhite, fontSize: 16, lineHeight: 1.8 }}><strong>Phone:</strong> {SITE.phone}<br /><strong>Email:</strong> {SITE.email}</p>
      </div>
      <div style={{ padding: mobile ? "40px 20px" : "60px 40px", background: C.bgAlt, textAlign: "center" }}>
        <h2 style={{ fontSize: mobile ? 22 : 28, fontWeight: 700, margin: "0 0 24px", color: C.white }}>Service Times</h2>
        <TimesGrid />
      </div>
    </>
  );
...
function GivePage() {
  ...
  return (
    <>
      ...
      <div style={{ padding: mobile ? "40px 20px" : "60px 40px", background: C.bgAlt }}>
        <div style={{ maxWidth: 740, margin: "0 auto" }}>
          <h2 style={{ fontSize: mobile ? 20 : 24, fontWeight: 700, marginBottom: 12, color: C.white }}>Why we give.</h2>
          <p style={{ color: C.offWhite, fontSize: 15, lineHeight: 1.7, marginBottom: 32 }}>God is generous and so he calls us to be as well. What we do with what God has given us shows the world where our hearts are at and helps proclaim the gospel. We want to glorify God with every area of our lives, and that includes what we do with our finances.</p>
          <h2 style={{ fontSize: mobile ? 20 : 24, fontWeight: 700, marginBottom: 20, color: C.white }}>Ways To Give</h2>
          {ways.map((w, i) => (
...
function SermonsPage() {
  ...
  const grouped = {};
  SITE.sermons.forEach(s => { if (!grouped[s.series]) grouped[s.series] = []; grouped[s.series].push(s); });
  return (
    <>
        ...
        <div style={{ maxWidth: 700, margin: "0 auto" }}>
          {Object.entries(grouped).map(([series, sermons]) => (
            <div key={series} style={{ marginBottom: 32 }}>
              ...
              {sermons.map((s, i) => (
                <div key={i} style={{ background: C.bgLight, borderRadius: 8, padding: "18px 20px", marginBottom: 10, border: `1px solid ${C.border}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
                    ...
                    {s.audioUrl ? (
                      <button onClick={() => setPlaying(playing === i ? null : i)} style={{ ...btnSt, padding: "8px 18px", fontSize: 13, borderRadius: 20 }}>
                  ...
                  {playing === i && s.audioUrl && <audio controls autoPlay src={s.audioUrl} style={{ width: "100%", marginTop: 12 }} />}
                </div>
              ))}
            </div>
          ))}
      ...
      <div style={{ padding: mobile ? "40px 20px" : "60px 40px", background: C.bg, textAlign: "center" }}>
        <h2 style={{ fontSize: mobile ? 20 : 24, fontWeight: 700, marginBottom: 8, color: C.white }}>Get The App</h2>
      ...
      <div style={{ padding: mobile ? "40px 20px" : "60px 40px", background: C.bgAlt, textAlign: "center" }}>
        {hasEmbed ? (
          <div style={{ maxWidth: 700, margin: "0 auto", borderRadius: 8, overflow: "hidden" }}>
            <iframe src={SITE.livestreamEmbedUrl} style={{ width: "100%", aspectRatio: "16/9", border: "none" }} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
          </div>
        ) : (
          <div style={{ maxWidth: 700, margin: "0 auto", background: C.bgDark, borderRadius: 8, aspectRatio: "16/9", display: "flex", alignItems: "center", justifyContent: "center", border: `1px solid ${C.border}`, flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ marginTop: 24, display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
          {SITE.youtubeChannel && <a href={SITE.youtubeChannel} target="_blank" style={{ ...btnSt, textDecoration: "none", display: "inline-block" }}>Watch on YouTube</a>}
          {SITE.facebookPage && <a href={SITE.facebookPage} target="_blank" style={{ ...btnSt, background: "transparent", border: `2px solid ${C.accent}`, color: C.accent, textDecoration: "none", display: "inline-block" }}>Watch on Facebook</a>}
        </div>
      </div>
      <div style={{ padding: mobile ? "40px 20px" : "60px 40px", background: C.bg, textAlign: "center" }}>
        <h2 style={{ fontSize: mobile ? 20 : 24, fontWeight: 700, marginBottom: 8, color: C.white }}>Get The App</h2>
...
export default function IgniteMB() {
  ...
  return (
    <div style={{ fontFamily: ff, color: C.white, background: C.bg, minHeight: "100vh", lineHeight: 1.6 }}>
